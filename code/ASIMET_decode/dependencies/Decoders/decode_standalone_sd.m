...
%%  Modifications
% 2025/08/25 bgreenwood; add ASCII text output for longwave standalones (requested by J.Smith)
% 2025/06/10 bgreenwood; Add code to parse latest Lane Abrams firmware 'ASIEPLWR24 v6.0'
% 2021/09/27 bgreenwood; Jason Smith request - add ASCII text output for SWR SD standalone
% 2021/02/09 bgreenwood; add support for SD card Sonic
...
% 2003/08 Code by ngalbraith
    ...
    meta.decoder.version = '2025-06-10';
    ...
    switch lower(inst_type)
        ...
        case  {'lwr'}
            ...
            vtypes = {'uint16';'uint16';'float';'uint16'};
            vscale = [0.001,0.001,1,0.1];
            nbytes = 696;
            numunused = 12;
            ...
            meta.data.dome.unit='C';
            meta.data.body.long_name='Body Temperature';
            meta.data.body.unit='C';
        ...
        case {'asg'}
            vnames = {'wnde';'wndn';'wspd';'wspd_max';'last_xy';'last_compass';'tilt_x';'tilt_y';'gill_sos';'gill_tempC'};
        	...
        	meta.data.wspd_max.long_name='Maximum Wind Speed';
        	meta.data.last_xy.long_name='Last XY Direction';
        	meta.data.last_compass.long_name='Last Compass Direction';
        	meta.data.last_compass.units='degree';
        	meta.data.tilt_x.long_name='the average x-axis tilt';
        	meta.data.tilt_x.units='degree';
        	meta.data.tilt_y.long_name='the average y-axis tilt';
        	meta.data.tilt_y.units='degree';
        	meta.data.gill_sos.long_name='Speed of Sound';
        	meta.data.gill_sos.units='m/s';
        	meta.data.gill_tempC.long_name='Sonic temperature';
        	meta.data.gill_tempC.units='Celsius';
        otherwise
    ...
    for irec = 1:nhours
        try
            ...
            for v = 1:length(vnames)
                if strcmp(inst_type,'wnd') || strcmp(inst_type,'asg') || strcmp(inst_type,'lwr') % wind values have different data types and must be scaled
            ...
            if (CRC(irec,1) ~= used | CRC(irec,2) ~= used) & ~strcmp(inst_type,'lwr')
               fprintf('bad checksum %02X %02X\n',CRC(irec,1),CRC(irec,2));
    ...
    % 2021/09/27 BG add ASCII text output for shortwave standalones (requested by J.Smith)
    if strcmp(inst_type,'swr')
        ascii_path = [ DATADIR 'processed/' sa_file '.asc' ];
        aout = fopen(ascii_path,'w');
        for r = 1:length(mday)
            timestamp = datestr(mday(r),'yyyy mm dd HH MM SS');
            fprintf(aout,'%s      %.2f\n',timestamp,swr(r));
        end
        fclose(aout);
    end

    % 2025/08/25 BG add ASCII text output for longwave standalones (requested by J.Smith)
    if strcmp(inst_type,'lwr')
        ascii_path = [ DATADIR 'processed/' sa_file '.asc' ];
        aout = fopen(ascii_path,'w');
        for r = 1:length(mday)
            timestamp = datestr(mday(r),'yyyy mm dd HH MM SS');
            fprintf(aout,'%s      %6.2f %6.2f %7.3f %7.3f\n',timestamp,dome(r),body(r),tpile(r),lwr(r));
        end
        fclose(aout);
    end
